...
//------------------------------------------------------------------------------
task_manager::task_manager(lmf_event& ev) : event_sub_(ev) {
  terminate  = false;
  terminated = false;
  ...
  if (timerfd_settime(sfd, TFD_TIMER_ABSTIME, &its, NULL) == -1) {
    Logger::lmf_app().error("Failed to set timer for task manager");
  }
}
...
void task_manager::manage_tasks() {
  ...
  while (1) {
    ...
    wait_for_cycle();
    if (terminate) {
      Logger::lmf_app().debug("Exit loop in manage_tasks");
      terminated = true;
      return;
    }
  }
...
void task_manager::wait_for_cycle() {
  ...
  if (sfd > 0) {
    ...
    if ((res < 0) || (res != sizeof(exp))) {
      Logger::lmf_app().error("Failed in task manager timer wait");
    }
  }
}
