...
//------------------------------------------------------------------------------
void lmf_client::curl_http_client(std::string remoteUri, std::string method,
  ...
  if (curl) {
    ...
    if (httpCode == 0) {
      Logger::lmf_app().info("Cannot get response when calling %s",
                             remoteUri.c_str());
    ...
    if (!is_response_ok) {
      ...
      Logger::lmf_app().info("Get response with jsonData: %s",
                             response.c_str());
