...
bool InputData::validate(std::stringstream &msg,
  ...
  if (reportingAccessTypesIsSet()) {
    const std::vector<ReportingAccessType> &value = m_ReportingAccessTypes;
    ...
    { // Recursive validation of array elements
      const std::string oldValuePath = currentValuePath;
      int i = 0;
      for (const ReportingAccessType &value : value) {
        const std::string currentValuePath =
            oldValuePath + "[" + std::to_string(i) + "]";
...
std::vector<ReportingAccessType> InputData::getReportingAccessTypes() const {
