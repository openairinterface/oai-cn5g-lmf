 ...
 */
int GeneralizedTime_constraint(
  ...
  if (tloc == -1 && errno != EPERM) {
    ASN__CTFAIL(
        app_key, td, sptr, "%s: Invalid time format: %s (%s:%d)", td->name,
...
asn_enc_rval_t GeneralizedTime_encode_der(
   ...
   */
  errno = EPERM;
  tloc  = asn_GT2time_frac(
      (const GeneralizedTime_t*) sptr, &fv, &fd, &tm, 1); /* Recognize time */
  if (tloc == -1 && errno != EPERM) {
...
asn_enc_rval_t GeneralizedTime_encode_xer(
...
int GeneralizedTime_print(
  ...
  if (st && st->buf) {
    ...
    ret = snprintf(
        buf, sizeof(buf), "%04d-%02d-%02d %02d:%02d:%02d (GMT)",
...
time_t asn_GT2time_prec(
    const GeneralizedTime_t* st, int* frac_value, int frac_digits,
...
GeneralizedTime_t* asn_time2GT(
    GeneralizedTime_t* opt_gt, const struct tm* tm, int force_gmt) {
  return asn_time2GT_frac(opt_gt, tm, 0, 0, force_gmt);
}

GeneralizedTime_t* asn_time2GT_frac(
    GeneralizedTime_t* opt_gt, const struct tm* tm, int frac_value,
  ...
  size = snprintf(
      buf, buf_size, "%04d%02d%02d%02d%02d%02d", tm->tm_year + 1900,
  ...
  if (force_gmt) {
    ...
    *p++ = 0;
    ...
    gmtoff %= 86400;
    ret = snprintf(
        p, buf_size - size, "%+03ld%02ld", gmtoff / 3600,
        labs(gmtoff % 3600) / 60);
    if (ret != 5) {
      FREEMEM(buf);
      errno = EINVAL;
