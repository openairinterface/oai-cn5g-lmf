/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "NRPPA-IEs"
 * 	found in "nrppa.asn1"
 * 	`asn1c -pdu=all -fcompound-names -fno-include-deps -findirect-choice
 * -no-gen-BER -no-gen-OER -no-gen-JER -no-gen-example -D oai-lmf/src/nrppa/`
 */

#include "E-CID-MeasurementResult.h"

#include "NG-RANAccessPointPosition.h"
#include "MeasuredResults.h"
#include "ProtocolExtensionContainer.h"
asn_TYPE_member_t asn_MBR_E_CID_MeasurementResult_1[] = {
    {ATF_NOFLAGS,
     0,
     offsetof(struct E_CID_MeasurementResult, servingCell_ID),
     (ASN_TAG_CLASS_CONTEXT | (0 << 2)),
     -1, /* IMPLICIT tag at current level */
     &asn_DEF_NG_RAN_CGI,
     0,
     {
#if !defined(ASN_DISABLE_OER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_OER_SUPPORT) */
#if !defined(ASN_DISABLE_UPER_SUPPORT) || !defined(ASN_DISABLE_APER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_UPER_SUPPORT) ||                                \
          !defined(ASN_DISABLE_APER_SUPPORT) */
         0},
     0,
     0, /* No default value */
     "servingCell-ID"},
    {ATF_NOFLAGS,
     0,
     offsetof(struct E_CID_MeasurementResult, servingCellTAC),
     (ASN_TAG_CLASS_CONTEXT | (1 << 2)),
     -1, /* IMPLICIT tag at current level */
     &asn_DEF_TAC,
     0,
     {
#if !defined(ASN_DISABLE_OER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_OER_SUPPORT) */
#if !defined(ASN_DISABLE_UPER_SUPPORT) || !defined(ASN_DISABLE_APER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_UPER_SUPPORT) ||                                \
          !defined(ASN_DISABLE_APER_SUPPORT) */
         0},
     0,
     0, /* No default value */
     "servingCellTAC"},
    {ATF_POINTER,
     3,
     offsetof(struct E_CID_MeasurementResult, nG_RANAccessPointPosition),
     (ASN_TAG_CLASS_CONTEXT | (2 << 2)),
     -1, /* IMPLICIT tag at current level */
     &asn_DEF_NG_RANAccessPointPosition,
     0,
     {
#if !defined(ASN_DISABLE_OER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_OER_SUPPORT) */
#if !defined(ASN_DISABLE_UPER_SUPPORT) || !defined(ASN_DISABLE_APER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_UPER_SUPPORT) ||                                \
          !defined(ASN_DISABLE_APER_SUPPORT) */
         0},
     0,
     0, /* No default value */
     "nG-RANAccessPointPosition"},
    {ATF_POINTER,
     2,
     offsetof(struct E_CID_MeasurementResult, measuredResults),
     (ASN_TAG_CLASS_CONTEXT | (3 << 2)),
     -1, /* IMPLICIT tag at current level */
     &asn_DEF_MeasuredResults,
     0,
     {
#if !defined(ASN_DISABLE_OER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_OER_SUPPORT) */
#if !defined(ASN_DISABLE_UPER_SUPPORT) || !defined(ASN_DISABLE_APER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_UPER_SUPPORT) ||                                \
          !defined(ASN_DISABLE_APER_SUPPORT) */
         0},
     0,
     0, /* No default value */
     "measuredResults"},
    {ATF_POINTER,
     1,
     offsetof(struct E_CID_MeasurementResult, iE_Extensions),
     (ASN_TAG_CLASS_CONTEXT | (4 << 2)),
     -1, /* IMPLICIT tag at current level */
     &asn_DEF_ProtocolExtensionContainer_2920P16,
     0,
     {
#if !defined(ASN_DISABLE_OER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_OER_SUPPORT) */
#if !defined(ASN_DISABLE_UPER_SUPPORT) || !defined(ASN_DISABLE_APER_SUPPORT)
         0,
#endif /* !defined(ASN_DISABLE_UPER_SUPPORT) ||                                \
          !defined(ASN_DISABLE_APER_SUPPORT) */
         0},
     0,
     0, /* No default value */
     "iE-Extensions"},
};
static const int asn_MAP_E_CID_MeasurementResult_oms_1[]            = {2, 3, 4};
static const ber_tlv_tag_t asn_DEF_E_CID_MeasurementResult_tags_1[] = {
    (ASN_TAG_CLASS_UNIVERSAL | (16 << 2))};
static const asn_TYPE_tag2member_t asn_MAP_E_CID_MeasurementResult_tag2el_1[] =
    {
        {(ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0}, /* servingCell-ID */
        {(ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0}, /* servingCellTAC */
        {(ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0,
         0}, /* nG-RANAccessPointPosition */
        {(ASN_TAG_CLASS_CONTEXT | (3 << 2)), 3, 0, 0}, /* measuredResults */
        {(ASN_TAG_CLASS_CONTEXT | (4 << 2)), 4, 0, 0}  /* iE-Extensions */
};
asn_SEQUENCE_specifics_t asn_SPC_E_CID_MeasurementResult_specs_1 = {
    sizeof(struct E_CID_MeasurementResult),
    offsetof(struct E_CID_MeasurementResult, _asn_ctx),
    asn_MAP_E_CID_MeasurementResult_tag2el_1,
    5,                                     /* Count of tags in the map */
    asn_MAP_E_CID_MeasurementResult_oms_1, /* Optional members */
    3,
    0, /* Root/Additions */
    5, /* First extension addition */
};
asn_TYPE_descriptor_t asn_DEF_E_CID_MeasurementResult = {
    "E-CID-MeasurementResult",
    "E-CID-MeasurementResult",
    &asn_OP_SEQUENCE,
    asn_DEF_E_CID_MeasurementResult_tags_1,
    sizeof(asn_DEF_E_CID_MeasurementResult_tags_1) /
        sizeof(asn_DEF_E_CID_MeasurementResult_tags_1[0]), /* 1 */
    asn_DEF_E_CID_MeasurementResult_tags_1,                /* Same as above */
    sizeof(asn_DEF_E_CID_MeasurementResult_tags_1) /
        sizeof(asn_DEF_E_CID_MeasurementResult_tags_1[0]), /* 1 */
    {
#if !defined(ASN_DISABLE_OER_SUPPORT)
        0,
#endif /* !defined(ASN_DISABLE_OER_SUPPORT) */
#if !defined(ASN_DISABLE_UPER_SUPPORT) || !defined(ASN_DISABLE_APER_SUPPORT)
        0,
#endif /* !defined(ASN_DISABLE_UPER_SUPPORT) ||                                \
          !defined(ASN_DISABLE_APER_SUPPORT) */
        SEQUENCE_constraint},
    asn_MBR_E_CID_MeasurementResult_1,
    5,                                       /* Elements count */
    &asn_SPC_E_CID_MeasurementResult_specs_1 /* Additional specs */
};
