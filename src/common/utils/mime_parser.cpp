...
bool mime_parser::parse(const std::string& str) {
  ...
  Logger::lmf_server().debug("Boundary: %s", boundary_str.c_str());
  ...
  while (boundary_pos < boundary_last_post) {
    ...
    Logger::lmf_server().debug("Content Type: %s", p.content_type.c_str());
    ...
    if (boundary_pos > 0) {
      ...
      Logger::lmf_server().debug("Body: %s", p.body.c_str());
...
unsigned char* mime_parser::format_string_as_hex(const std::string& str) {
  ...
  Logger::lmf_server().debug(
      ...
      str.c_str());
  Logger::lmf_server().debug("Data (formatted):");
#if DEBUG_IS_ON
