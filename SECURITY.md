...
- **Email subject**: `[SEC-VUL-OAI-CN5G-LMF]: "Mention the affected
  functionality here"`.
...
in the
[GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-lmf/issues),
...
- **Issue title**: `[AI-SEC-VUL-OAI-CN5G-LMF]: "Mention the affected
  functionality here"`.
- **Issue label**: apply the
  https://github.com/openairinterface/oai-cn5g-lmf/labels/security
...
The [openairinterface/oai-cn5g-lmf](https://github.com/openairinterface/oai-cn5g-lmf)
repository contains the OAI implementation of the 5G Core **Location Management
Function (LMF)**, together with its configuration templates, container images,
...
NRF, PCF, NSSF, NEF, NWDAF), in the federation repository
...
can be reported here if they are reachable through the LMF. In that case, please
state clearly which component is affected and give the commit ID of the affected
submodule in addition to the LMF commit.

Security reports are in scope when they affect the confidentiality, integrity,
or availability of the LMF running in a documented or reasonably expected
deployment.

In-scope examples include:

- Memory corruption, crashes, infinite loops, resource exhaustion, or denial of
  service triggered by malformed or unexpected LPP (3GPP TS 37.355) or NRPPa
  (3GPP TS 38.455) payloads, or by malformed SBI (HTTP/1.1, HTTP/2, JSON,
  multipart/MIME) requests or notifications, including ASN.1 decoding defects.
  These payloads reach the LMF through SBI messages relayed by the AMF, so a
  malicious UE or NG-RAN node may be a realistic source.
- Defects in the implemented LMF service operations (`DetermineLocation` and the
  UE and non-UE N2 information notification callbacks) that allow a consumer to
  obtain the location of a target UE without appropriate authorisation, inject
  or replay positioning payloads into another UE's session, or hijack or
  interfere with an existing positioning procedure through forged NRPPa
  transaction, measurement, LCS correlation, or N2 notification subscription
  identifiers (3GPP TS 23.273).
- Exposure of subscriber or UE identifiers, including SUPI/IMSI where present,
  UE location estimates, or positioning measurements through logs, statistics,
  notification URIs, API responses, or error paths.
...
- Bugs requiring local admin/root access on the LMF host with no privilege
  boundary crossed.
...
- Vulnerabilities solely caused by third-party projects or dependencies should
  ...
  the LMF deployment or requires a change in this repository, please report it
  here as well.
...
- Denial of service that only depends on flooding a plaintext, unprotected
  ...
  deployment, rather than on an LMF software defect.
